...
namespace GameDevTV.Inventories
{
    ...
    public class Inventory : MonoBehaviour, ISaveable, IPredicateEvaluator
    {
        ...
        public bool HasItemByID(string itemID)
        {
            for (int i = 0; i < slots.Length; i++)
            {
                if (slots[i].item != null && slots[i].item.GetItemID() == itemID)
                {
                    return true;
                }
            }
        ...
        {
            if (InventoryItem.GetFromID(itemID) == null)
            {
                Debug.LogWarning($"Invalid Item ID assigned: {itemID}");
            }
        }*/
        ...
        public void RemoveFromSlot(int slot, int number)
        {
            ...
            if (inventoryUpdated != null)
            {
                ...
                //Remove particular item specified in editor like scrip object
